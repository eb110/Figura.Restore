...
{
    //SignInManager have the 'logout' functionality
    public class AccountController(SignInManager<User> signInManager) : BaseApiController
    {
        ...
        {
            //the task of it is just to send user info - we don't have to rise 'Unathorized' call
            //in case the user is unathorized - simple 'nocontent' will be enough
            if(User.Identity?.IsAuthenticated == false)
            {
                return NoContent();
            }

            //User contains claims principals => capital 'U'ser
        ...
        {
            //signs out current user and removes the cookie from the browser
        ...
        public async Task<ActionResult<Address>> GetSavedAddress()
        {
            ...
            if (address == null)
            {
                //no content as user does not to have the address
                return NoContent();
            }
